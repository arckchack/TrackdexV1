...
# language_id 7 is Spanish, 9 is English
species_names = {}
for r in species_names_rows:
    sp_id = int(r['pokemon_species_id'])
    lang_id = int(r['local_language_id'])
    name = r['name']
    if sp_id not in species_names or lang_id == 7: # prefer Spanish if available
...
for r in item_names_rows:
    ...
    lang_id = int(r['local_language_id'])
    name = r['name']
    if i_id not in item_names or lang_id == 7:
        item_names[i_id] = name

# Build move names map
move_names = {}
...
# Triggers
# 1: level-up, 2: trade, 3: use-item, 4: shed, 5: other
TRIGGER_NAMES = {
    '1': 'Subir nivel',
    '2': 'Intercambio',
    '3': 'Usar objeto',
...
# Map evolved_species_id -> best evolution condition string
evolution_methods = {}

for r in evolution_rows:
    if r['is_default'] == '0' and r['evolved_species_id'] in evolution_methods:
        continue # skip non-default duplicates if already set
    ...
    if item_id and int(item_id) in item_names:
        parts.append(f"Usar {item_names[int(item_id)]}")
    ...
    if trig_id == '2':
        ...
        if held_item and int(held_item) in item_names:
            parts.append(f"Intercambio con {item_names[int(held_item)]}")
    ...
    if held_item and trig_id != '2' and int(held_item) in item_names:
        parts.append(f"Equipado con {item_names[int(held_item)]}")
    ...
    if r['turn_upside_down'] == '1':
        parts.append("Girar consola bocabajo")
...
print(f"Processed {len(evolution_methods)} evolution methods.")

# Now group species by evolution_chain_id
chain_species = {}
species_info = {}
...
# For each species_id -> return list of species in chain with relationships
...
print(f"Built evolution data for {len(pokemon_evolution_data)} Pokémon!")
...
print("Saved src/data/pokemonEvolutions.json successfully!")
